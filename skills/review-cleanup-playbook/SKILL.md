...
description: 'Use when a review should include actionable code-cleanup recommendations with Clean Code-style rule tags and small, low-risk refactor steps. Designed for the Reviewer.'
...
This skill adds a cleanup-first pass to reviews. It is based on the Python clean-code skill style from `ertugrul-dmr/clean-code-skills` and adapted for this project's conventions.
...
- Boundary hygiene: logic placed in the wrong layer, CLI handlers accumulating domain logic, format parsing outside the input layer, persistence logic outside the persistence layer.
...
- Follow module boundaries from `copilot-instructions.md` — do not invent new layering.
...
- `[G30]` `src/core/processor.py`: `process_records` mixes filtering, transformation, and output formatting. Split formatting into a dedicated helper to keep processing logic linear.
