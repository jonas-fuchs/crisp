...
description: 'Conducts multi-axis code review. Use before merging any change. Use when reviewing code written by yourself, another agent, or a human. Use when assessing code quality across correctness, readability, architecture, security, and performance before it enters the main branch.'
...
- Are intent comments present where the logic is non-obvious (coordinate conversions, domain-specific assumptions)?
...
- Does it respect the module boundaries defined in `copilot-instructions.md`?
...
For the full module boundary rules and project-specific architecture conventions, refer to `copilot-instructions.md`.

### 4. Security

Does the change introduce vulnerabilities?

- Is user input validated and sanitised at system boundaries (upload endpoints, CLI file paths, file content)?
...
For detailed security guidance and the full checklist, use the `security-and-hardening` skill or the **Security** agent.
...
- Any large objects created in hot paths (processing loops, per-item annotation)?
- Are results cached where caching is expected?
...
- Are edge cases covered (boundary values, ambiguous states, empty inputs, None)?- Are deterministic output tests stable?
...
When the diff changes module boundaries or cross-layer flows, use `graphify` to verify architecture drift and identify any required updates to development layout docs.
...
- [ ] Edge cases handled (boundary, empty, ambiguous states)
- [ ] Error paths handled
...
### Architecture
- [ ] Module boundaries respected (see copilot-instructions.md)
